...
def callback(data):
    # Do GP stuff here *********
    rospy.loginfo("GP Input Arrived")
    X = data.time_array
    #X = [1,2,3,4,5]
...
# X_=np.arange(X.min(),X.max()+50, 0.1)
    ...
    # [6,7,8,9] # for example
    msg_out.sigma = 2 * np.sqrt(np.array(variances[int(len(X)):]))
    ...
    # # confidence intervals #C0C0C0
    # plt.fill_between(X_, var2[0, :], var1[0, :], alpha=0.3, color='k')
